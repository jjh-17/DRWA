...
function joinSession() {
  ...
  getToken().then((token) => {
    ...
    sessionInfo.session
      .connect(token, {
        clientData: `${playerInfo.memberId},${playerInfo.nickname},${playerInfo.team}`
      })
      .then(() => {
...
function changeTeam(event, targetTeam) {
  ...
  if (
    targetTeam != team[0].english &&
    targetTeam != team[1].english &&
    targetTeam != team[2].english &&
  ...
  // 자리가 없다면 작업 취소
  if (
    (targetTeam == team[0].english && sessionInfo.teamLeftList.length == roomInfo.playerNum / 2) ||
...
function leaveSession() {
  ...
  // 플레이어 정보 초기화
  ;(playerInfo.team = team[3].english), (playerInfo.order = -1)
  ...
  <div class="app-container">
    ...
    <div class="main-container">
      ...
      <div class="share-container">
        ...
        <div
          v-if="playerInfo.team != team[2].english"
          class="juror-button"
          @click="(event) => changeTeam(event, team[2].english)"
        >
          배심원으로 입장 ( / )
        </div>
        ...
        <div
          v-if="playerInfo.team != team[3].english"
          class="viewer-button"
